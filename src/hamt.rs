...
impl<'a, K: PartialEq, V: PartialEq, S: Store> PartialEq for Hamt<'a, K, V, S> {
    ...
    S: Store,
{
    ...
    pub fn from_link(cid: &TaggedCid, store: &'a S) -> Result<Self> {
        match store.get(cid)? {
            Some(root) => Ok(Hamt { root, store }),
            None => Err(format_err!("No node found")),
        }
    }
    ...
    ///
    /// let store = hamt_ipld::store::MemoryStore::default();
...
where
    ...
    V: Serialize + DeserializeOwned,
{
    pub fn insert<S: Store>(&mut self, key: K, value: V, store: &S) -> Option<V> {
    ...
    #[inline]
    pub fn get<Q: ?Sized, S: Store>(&self, k: &Q, store: &S) -> Option<&V>
    where
        K: Borrow<Q>,
        Q: Eq + Hash,
    {
    ...
    /// Search for a key.
    #[inline]
    fn search<Q: ?Sized, S: Store>(&self, q: &Q, store: &S) -> Option<&KeyValuePair<K, V>>
    where
        K: Borrow<Q>,
        Q: Eq + Hash,
    {
    ...
    fn modify_value<S: Store>(
        ...
        ensure!(depth < hashed_key.len(), "Maximum depth reached");
        let idx = hashed_key[depth];
    ...
    pub fn rm_value<Q: ?Sized, S: Store>(
        &mut self,
        hashed_key: HashedKey,
        depth: usize,
        key: &Q,
        ...
        Q: Hash + Eq,
    {
        ...
        // No existing values at this point.
        ensure!(self.bitfield.test_bit(idx), "Not found");
...
where
    ...
    V: Serialize + DeserializeOwned,
{
    pub fn from_link(link: TaggedCid, node: Node<K, V>) -> Self {
    ...
    fn load_child<S: Store>(&self, store: &S) -> Result<&Node<K, V>> {
        if !self.cache.filled() {
            if let Some(ref link) = self.link {
                match store.get(link)? {
                    ...
                    None => bail!("node not found"),
                }
            } else {
                bail!("Cannot load child from non link node");
            }
        }
        Ok(self.cache.borrow().unwrap())
    }

    fn load_child_mut<S: Store>(&mut self, store: &S) -> Result<&mut Node<K, V>> {
        if !self.cache.filled() {
            if let Some(ref link) = self.link {
                match store.get(link)? {
                    Some(node) => {
...
mod tests {
    ...
    fn test_from_link() {
        ...
        assert_eq!(hamt.get(&1), Some(&"world2".to_string()));
        let c = store.insert(&hamt).unwrap();
        ...
        // loading new hash
        let c2 = store.insert(&hamt).unwrap();
        ...
        let c3 = store.insert(&hamt.root).unwrap();
        assert_eq!(c3, c2);
    }
}
